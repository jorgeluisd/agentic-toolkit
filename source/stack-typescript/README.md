...
|---|---|---|
| `templates/settings.json` | `{{SETTINGS_FILE}}` | Permisos del proyecto: `allow` para typecheck/lint/test/build y git de lectura y commit; `ask` para push, PR, instalación de paquetes, migraciones, `psql`, deploys; `deny` para `npm`/`yarn`, `--force`, `--no-verify`, `gh pr merge` y toda lectura o escritura de `.env*`, `*.pem`, `*.key`. Además fija `includeCoAuthoredBy: false` |
...
Copiar y adaptar; ninguna se instala sola. El comando `{{CMD_PREFIX}}adopt` las aplica como parte de la migración.
