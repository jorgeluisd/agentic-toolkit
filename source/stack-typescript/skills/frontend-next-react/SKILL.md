...
El dialecto del copy de usuario (neutro, regional, tuteo/ustedeo) lo define el `{{CONTEXT_DOC}}` de cada producto; esta skill no lo fija. Identificadores de código siempre en inglés.
...
import { copy } from '@/copy'; // dialecto según {{CONTEXT_DOC}} del producto
...
- [ ] ¿Errores mapeados por `code` con copy del producto (dialecto según su `{{CONTEXT_DOC}}`)?
