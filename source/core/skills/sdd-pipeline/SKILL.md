...
Cuando el usuario pide un cambio de código en lenguaje natural ("agrega un endpoint", "arregla el bug de X", "implementa la feature Y", "migra la tabla Z"), **no se implementa directo**. Se hace exactamente lo que haría `{{CMD_PREFIX}}sdd <objetivo>`: leer `{{ORCHESTRATOR}}` del plugin, clasificar el nivel y correr las fases en orden con artefactos en `docs/sdd/<NNNN>-<slug>/`.
