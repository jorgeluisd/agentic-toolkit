...
5. Checks locales al cerrar la tarea: typecheck, lint (boundaries) y los tests de los archivos tocados. Los comandos son los declarados en la sección "Comandos" del `{{CONTEXT_DOC}}` del proyecto.
...
7. Registra en `05-apply-progress.md` (plantilla abajo). **Si el archivo ya acumula más de `SDD_PROGRESS_KEEP_TASKS` tareas (default 10), rotalo antes de seguir**: mové el detalle de las más viejas a `05-apply-progress-T<n>-T<m>.md` y dejá en el archivo vivo una tabla `RESUMEN ACUMULADO` (una fila por tarea archivada: `T-n · estado · commit · AC cubiertos`) más el detalle completo de las últimas. Vos releés este archivo en cada tarea: sin rotar, la tarea N paga el detalle de las N-1 anteriores. El gatekeeper deniega tu lanzamiento hasta que esté rotado. El hook `tdd-evidence` registra automáticamente cada corrida de tests en `tdd-evidence.log`; tu tabla referencia sus timestamps. **Cada fila se llena copiando la línea del log** (buscala con `grep`): timestamp exacto y resumen literal del runner. Nunca `idem`, `(mismo)`, `04:0x` ni un timestamp de memoria: el `verifier` contrasta fila contra línea y una fila sin línea es evidencia no respaldada. Si la corrida quedó con `WARN=backgrounded` o `WARN=compile-error`, no es el RED de la fila: repetila en primer plano (`strict-tdd` §3 y §6).
