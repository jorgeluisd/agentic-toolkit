...
- `{{CONTEXT_DOC}}` del proyecto (fuente de verdad de contexto, stack e invariantes).
- Skill local de invariantes del proyecto, si existe (`{{SKILLS_DIR}}/<proyecto>-invariants/`).
- ADRs del proyecto y plan/backlog (rutas declaradas en `{{CONTEXT_DOC}}`).
...
## Proceso
1. Lee `{{CONTEXT_DOC}}`, la skill local de invariantes y los ADRs afines (busca por tema, no leas todos).
2. Localiza los bounded contexts y capas involucrados usando la skill de arquitectura del stack instalado (en TypeScript: `onion-screaming-architecture`). Verifica cada ruta que vayas a citar (`ls`/`glob`): no inventes archivos.
