...
Pipeline de 10 agentes para construir software con especificación verificable, diseño explícito, TDD estricto y dos paradas humanas. Agnóstico de producto y de lenguaje: el contexto de negocio y las invariantes viven en el `{{CONTEXT_DOC}}` y en la skill local de invariantes de cada proyecto; las reglas técnicas (capas, dominio, persistencia, tests, seguridad) las aporta el plugin de stack instalado junto a este core (`stack-typescript` u otro). Este documento define solo el proceso.
...
El pipeline se dispara igual si el pedido llega por `{{CMD_PREFIX}}sdd` o en lenguaje natural ("agrega un endpoint…"): la skill `sdd-pipeline` y el hook `UserPromptSubmit` del plugin recuerdan clasificar el nivel antes de tocar archivos, y el `{{CONTEXT_DOC}}` de cada proyecto lo fija como regla.
...
`SDD_ARTIFACT_STORE` elige la política; `SDD_ARTIFACTS_DIR` elige dónde se materializan los archivos. Ambos se declaran en `{{CONFIG_FILE}}` del repo (o el `userConfig` del plugin como fallback).
...
| `local` | `{{LOCAL_STORE}}/` | Fuera del registro: el repo queda intacto. Se pierde el compartir con el equipo y la auditoría del gate |
| `engram` | `{{LOCAL_STORE}}/` | Los artefactos narrativos van a memoria persistente bajo la misma clave `sdd/<change>/<artefacto>`; en disco queda solo lo que los hooks necesitan. Sin memoria conectada degrada a `local` |
...
3. **No romper lo verde.** Cada tarea deja typecheck, lint y tests en verde. Ese gate corre **al cerrar la tarea**, no dentro del ciclo: en RED/GREEN/TRIANGULATE va el test dirigido al archivo o al caso. Una suite completa con un RED abierto queda marcada con `WARN=full-suite-mid-cycle` y no cuenta como evidencia. Si el safety net falla antes de tocar nada, se reporta como falla preexistente y se para.
