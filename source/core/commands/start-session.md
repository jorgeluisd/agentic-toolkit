---
description: Arranca una sesión leyendo el estado local del proyecto ({{CONTEXT_DOC}}, ADRs, plan/backlog, último 08-close.md) y reporta dónde estamos antes de tocar código.
argument-hint: "[área o ítem a tocar | vacío para sesión exploratoria]"
---

Arranca la sesión de trabajo. Regla de oro (local-first): las decisiones y el estado viven en el repositorio; no asumas fuentes externas salvo que el `{{CONTEXT_DOC}}` las declare.

Lee, en este orden y solo lo necesario:
1. `{{CONTEXT_DOC}}` del proyecto (contexto, stack, invariantes, comandos, rutas de plan/backlog/ADRs).
