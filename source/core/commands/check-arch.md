...
Verifica que la arquitectura y el código estén sanos. Usa los comandos declarados en la sección "Comandos" del `{{CONTEXT_DOC}}` del proyecto (`typecheck`, `lint`, `test`, `test:integration`, `audit`). Si no están declarados, usa los que documente la skill `testing-conventions` del stack instalado; si tampoco existe, detente y pídelos: no adivines.
