...
t "store local"                            "$(root SDD_ARTIFACT_STORE=local)"        /proj/.agentic/sdd
...
# --------------------------------------------- config: .agentic/ y compatibilidad
# El nombre neutral es .agentic/; .claude/ se sigue aceptando para los repos
# adoptados antes de 2.0.0, y .agentic/ gana cuando están los dos.
sec "Archivo de configuración · orden de búsqueda"
E="$(mktemp -d "${TMPDIR:-/tmp}/sdd-env.XXXXXX")"
mkdir -p "$E/solo-agentic/.agentic" "$E/solo-claude/.claude" "$E/ambos/.agentic" "$E/ambos/.claude"
for d in solo-agentic solo-claude ambos; do git -C "$E/$d" init -q 2>/dev/null || { mkdir -p "$E/$d"; git -C "$E/$d" init -q; }; done
printf 'SDD_BASE_BRANCH=desde-agentic\n' > "$E/solo-agentic/.agentic/sdd-hooks.env"
printf 'SDD_BASE_BRANCH=desde-claude\n'  > "$E/solo-claude/.claude/sdd-hooks.env"
printf 'SDD_BASE_BRANCH=gana-agentic\n'  > "$E/ambos/.agentic/sdd-hooks.env"
printf 'SDD_BASE_BRANCH=pierde-claude\n' > "$E/ambos/.claude/sdd-hooks.env"
bb(){ env -u SDD_BASE_BRANCH SDD_PROJECT_DIR="$1" \
      bash -c ". $HOOKS/common.sh; printf '%s' \"\$BASE_BRANCH\""; }
t "lee .agentic/sdd-hooks.env"             "$(bb "$E/solo-agentic")"  desde-agentic
t "sigue leyendo .claude/sdd-hooks.env"    "$(bb "$E/solo-claude")"   desde-claude
t ".agentic gana sobre .claude"            "$(bb "$E/ambos")"         gana-agentic

mkdir -p "$E/store-viejo/.claude/sdd"
git -C "$E/store-viejo" init -q
lstore(){ env -u SDD_ARTIFACTS_DIR SDD_ARTIFACT_STORE=local SDD_PROJECT_DIR="$1" \
      bash -c ". $HOOKS/common.sh; printf '%s' \"\$ARTIFACTS_ROOT\""; }
t "store local respeta .claude/sdd previo" "$(lstore "$E/store-viejo")"   "$E/store-viejo/.claude/sdd"
rm -rf "$E"
