...
**Cuándo aplica (trigger de la entrada de Knowledge):** Baseline de seguridad verificable: OWASP con mitigación mecánica, JWT de un algoritmo, env con zod, webhooks HMAC, validación strict, SSRF allowlist, headers y rate limit con test, PII fuera de artefactos, cifrado envelope. Usar cuando se toca auth, inputs, secretos, webhooks, PII o se revisa un PR.
