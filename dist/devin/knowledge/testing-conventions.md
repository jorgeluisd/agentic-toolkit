...
**Cuándo aplica (trigger de la entrada de Knowledge):** Cuatro niveles de test con sufijos y comandos, Vitest único runner también en NestJS, cobertura por capa, fakes de puertos en vez de mocks, aserción de code, tests de tipos, integración contra Postgres real y e2e acotado. Usar cuando se escribe, ubica o revisa un test o se configura Vitest.
...
## 1b. Comandos simbólicos del stack (los usan `strict-tdd`, `delivery-workflow`, el `verifier` y `/check-arch`)

| Símbolo | Comando en este stack |
|---|---|
| `<install>` | `pnpm install --frozen-lockfile` |
| `<lint>` | `pnpm lint` (incluye `eslint-plugin-boundaries`) |
| `<typecheck>` | `pnpm typecheck` (`tsc --noEmit` en cada paquete) |
| `<test>` | `pnpm test` (unit + handler, sin Docker) |
| `<test:file>` | `pnpm --filter <pkg> exec vitest run <archivo.spec.ts> -t "<caso>"` |
| `<test:integration>` | `pnpm test:integration` (Postgres real, rol de app) |
| `<build>` | `pnpm build` |
| `<audit>` | `pnpm audit --audit-level=high` |

Cada proyecto los copia en su `AGENTS.md` §6; si difieren (turbo, filtros), gana el `AGENTS.md`. Un símbolo que el proyecto no tiene se declara `no aplica` con el porqué: sin declarar, el `verifier` no puede distinguir una decisión de un olvido (check 12).
