...
**Cuándo aplica (trigger de la entrada de Knowledge):** Familias DomainError/ApplicationError/InfrastructureError con code y httpStatus, publicMessage sin PII, Result<T,E> en domain/application, unwrapOrThrow en presentation y caza del Result descartado. Usar cuando se crea un error, se elige un status HTTP o se desempaca un Result.
...
| `publicMessage` | `string` | Va al body de la respuesta: sin PII, sin ids internos, sin jerga técnica, en el idioma del producto (dialecto según el AGENTS.md del proyecto). Default genérico por familia: olvidar el copy da un mensaje pobre, nunca una fuga |
