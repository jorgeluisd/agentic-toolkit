# application-cqrs-jobs

**Cuándo aplica (trigger de la entrada de Knowledge):** Commands/Queries con { input } único y tenant en ctx, handlers, puertos + Symbol, eventos con outbox, jobs pg-boss idempotentes por tenant, controllers finos con zod strict y mappers. Usar cuando se escribe un handler, use case, evento, job, controller o mapper.
