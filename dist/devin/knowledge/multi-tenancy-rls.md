...
**Cuándo aplica (trigger de la entrada de Knowledge):** Aislamiento por tenant: tenant desde auth y nunca del body, TenantContext.run con SET LOCAL, RLS ENABLE+FORCE+USING+WITH CHECK fail-closed, roles sin BYPASSRLS, autorización doble, 404 vs 403, jobs por tenant y tests cross-tenant. Usar cuando se toca una tabla, repo, guard o endpoint con tenant.
