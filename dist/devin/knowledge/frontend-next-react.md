...
**Cuándo aplica (trigger de la entrada de Knowledge):** Next.js App Router con Server Components por defecto, React 19 sin memo manual, server-only, Server Actions con zod y origen, consumo del API por contracts, errores por code, dinero en unidad menor, Tailwind 4 @theme + cn(), PWA idempotente y budgets Lighthouse. Usar cuando se toca apps/web.
...
El dialecto del copy de usuario (neutro, regional, tuteo/ustedeo) lo define el `AGENTS.md` de cada producto; esta skill no lo fija. Identificadores de código siempre en inglés.
...
import { copy } from '@/copy'; // dialecto según AGENTS.md del producto
...
- [ ] ¿Errores mapeados por `code` con copy del producto (dialecto según su `AGENTS.md`)?
