...
**Cuándo aplica (trigger de la entrada de Knowledge):** Protocolo RED-GREEN-TRIANGULATE-REFACTOR de 7 fases con comandos simbólicos del stack, RED válido y de tipos, estricto en domain/application y pragmático en adapters/UI, evidencia atada a tdd-evidence.log. Usar cuando se implementa una tarea con Strict TDD ON o se audita su evidencia.
...
Esta skill es agnóstica de lenguaje: los comandos aparecen como símbolos y el comando real lo define cada proyecto en `AGENTS.md` §6 y la skill `testing-conventions` del stack instalado. Símbolos: `<test:file>` (corre un archivo de test y, opcionalmente, un solo caso: `<test:file> <archivo> "<caso>"`), `<test>` (unit + handler, sin infraestructura externa), `<test:integration>` (contra base de datos/colas reales; requiere el entorno levantado), `<typecheck>` (chequeo estático: compilador o analizador estático del stack), `<lint>` (linter, incluidas reglas de límites entre capas). Resuélvelos antes del primer ciclo y úsalos literalmente en la evidencia; si el `AGENTS.md` no define uno, detente y pregunta, nunca inventes el comando.
...
| RED de API: la suite no compila porque el test usa un símbolo o un campo que el diseño declara y todavía no existe, y el error lo nombra (§3) | La suite no compila por un typo, un import mal escrito o un símbolo que no está en el diseño |
...
**RED de API (de compilación).** Un test que nombra un símbolo o un campo que el diseño declara y todavía no existe no compila: la suite corta antes de ejecutar un solo caso (`Tests: 0 total`, `error TS2305`, `TS2353`) y el hook marca la línea `WARN=compile-error`. Cuenta como RED **solo** si el error nombra ese símbolo o ese campo; un typo o un import mal escrito es RED inválido (§2). Y **no reemplaza al rojo de comportamiento**: el paso siguiente es el **esqueleto** —se crea la superficie (export, firma, tipo, campo) con el cuerpo que no cumple: devuelve `undefined`, lanza "not implemented"—, la suite vuelve a compilar y el mismo test falla por comportamiento con `≥1 failed`. Ese es el RED que la tabla referencia; la línea `compile-error` va como fila previa opcional, nunca sola.

**Reconstruir evidencia** (revertir producción y dejar los tests nuevos) se hace hasta el esqueleto, no hasta antes de la API: revertir del todo deja una suite que no compila, sin ningún rojo de comportamiento que mostrar.
...
El hook `post-bash` del plugin registra cada corrida de test en `docs/sdd/<feature>/tdd-evidence.log`, una línea por corrida: `<ISO-8601> | exit=<n> | <comando> | <resumen del runner>[ | WARN=<marca>]`. Está registrado en `PostToolUse` (la llamada Bash terminó en 0) y en `PostToolUseFailure` (falló), así que un RED queda con su exit code real y el resumen del runner. Ningún evento trae el exit code del runner: el del éxito no trae ninguno y el del fallo trae el de **la llamada entera**. El hook decide en este orden: el resumen del runner (`N failed` ⇒ rojo; `N passed` sin fallos ⇒ verde), después el evento, y solo si el payload no dice de qué evento viene, una inferencia por palabras; las líneas JSON de log quedan fuera de ambas cosas. Si el runner reporta tests ejecutados y ninguno rojo pero otra parte del comando falló, la línea queda `exit=0` con `WARN=call-failed-outside-tests`; si los tests ni llegaron a correr, `WARN=no-tests-ran`; si el rojo es de compilación y ningún caso se ejecutó, `WARN=compile-error` (§3). Si la llamada pasó a background —por timeout o `run_in_background`— el evento llega al pasar y el resultado real no llega nunca: la línea queda `exit=?` con `WARN=backgrounded` y **no es evidencia**; la corrida se repite en primer plano, dirigida al archivo o al caso. Solo cuando no llega ningún evento del harness la corrida se concilia como `| exit=!0 | <comando> | sin salida capturada | WARN=resultado-inferido-por-ausencia-de-PostToolUse`, y la línea de salida del RED se toma de lo que devolvió el comando en la conversación.

**Una corrida de tests va sola en su llamada Bash.** Nunca `<test> && cat > artefacto <<'EOF'` ni `<test>; <escritura>`: el exit code de la llamada deja de ser el de los tests, y una escritura que falla tapa un GREEN. Los artefactos se escriben con la herramienta de escritura de archivos o en otra llamada; si hace falta un heredoc, con `>|`: `cat >| archivo <<'EOF'`. En zsh con `noclobber` —activo en las máquinas del equipo— `cat > archivo-existente` sale 1 con "file exists" (y `>>` sobre un archivo que no existe también falla); `>|` ignora `noclobber` en zsh y en bash.

La tabla del apply-progress **referencia esos timestamps**, no hashes de commit: la evidencia sobrevive a squash y rebase. En la columna Comando va el comando real ya resuelto (el que el hook registró), no el símbolo. Cada fila se llena **copiando la línea del log**, encontrada con `grep`: el timestamp exacto y el resumen literal del runner. Nunca `idem`, `(mismo)`, un timestamp aproximado ni uno reconstruido de memoria — el `verifier` contrasta fila contra línea y una fila sin línea es evidencia no respaldada.
...
| T2-cancel-order | GREEN | rejects cancelling an already cancelled order | <test:file> order.aggregate.<ext> "already cancelled" | PASS — 1 passed | 2026-09-03T14:05:40Z |
| T2-cancel-order | RED | emits OrderCancelled exactly once | <test:file> order.aggregate.<ext> "exactly once" | FAIL — expected length 1, received 0 | 2026-09-03T14:07:02Z |
| T2-cancel-order | GREEN | emits OrderCancelled exactly once | <test:file> order.aggregate.<ext> "exactly once" | PASS — 2 passed | 2026-09-03T14:08:15Z |
| T2-cancel-order | REFACTOR | order.aggregate.<ext> | <test:file> order.aggregate.<ext> | PASS — 2 passed | 2026-09-03T14:12:30Z |
```

Validez (la revisa el `verifier`): cada fila tiene un timestamp presente en el log con el mismo comando y exit code coherente (RED `exit=1`…`exit=n` o `exit=!0`, GREEN/REFACTOR `exit=0`); el RED de un test precede a su GREEN; TRIANGULATE agrega pares RED→GREEN; ninguna fila sin línea de salida. **Una línea del log con `WARN=no-tests-ran`, `WARN=piped-output`, `WARN=output-truncated`, `WARN=interrupted` o `WARN=backgrounded` no es evidencia**: una corrida sin tests ejecutados (filtro `-t` que no coincide con ningún nombre, archivo mal escrito, "No test files found", o un comando que falló antes de llegar al runner) no es ni GREEN ni RED; una salida filtrada con `| tail`/`| grep` pierde el resumen del runner; el harness recorta la salida de una llamada fallida a unos 10 000 caracteres desde el principio, y si el resumen quedó afuera no hay forma de saber qué falló; y una corrida interrumpida no terminó. En todos los casos se repite la corrida sola, sin pipe y con el test dirigido (el resumen debe mostrar `≥1 passed` o `≥1 failed`). `WARN=call-failed-outside-tests` **sí** es evidencia del resultado del runner, pero delata una corrida encadenada con otro comando. Evidencia inválida → la tarea vuelve al `implementer`. **Cómo se verifica:** `grep -E "\| exit=([1-9]|!0)" docs/sdd/<feature>/tdd-evidence.log | grep -vcE "WARN=.*(no-tests-ran|piped-output|output-truncated|interrupted|backgrounded|compile-error)"` ≥ número de filas RED de comportamiento, y `grep "<timestamp>" ...` devuelve una línea por cada fila de la tabla.
...
- [ ] ¿Símbolos de §0 resueltos desde `AGENTS.md` §6 / `testing-conventions`, y estado Strict TDD (ON/OFF, tipo, override) registrado en el apply-progress?
