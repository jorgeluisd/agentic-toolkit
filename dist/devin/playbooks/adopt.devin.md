...
Adopta el estándar SDD+TDD en el repositorio actual de punta a punta (configuración, AGENTS.md, skill de invariantes, lint, CI, feature piloto y limpieza de lo viejo), detectando los valores del proyecto y deteniéndose en cuatro checkpoints humanos.

**Argumentos:** `[--base=<rama>] [--store=repo|local|engram] [--tenant=<campo>|none] [--prod=<regex>] [--informe=<ruta>] [--skip-pilot]`

## Procedure


Migra este repositorio al estándar que definen `sdd-tdd-core` y el plugin de stack instalado. Trabajas en una rama `chore/adopt-sdd-toolkit` creada desde la rama base. Hasta el paso 8 conviven el orquestador, comandos y skills viejos del repo con los del plugin: **no uses los viejos**. No borras nada de `.agents/` antes del paso 8.

Argumentos: lo que te pidió el usuario
...
| Comandos simbólicos | `<install> <lint> <typecheck> <test> <test:file> <test:integration> <build> <audit>` desde los scripts del gestor de paquetes. **Todos salen del checkpoint con un valor**: el comando, o `no aplica` con el porqué. Sin `typecheck`, `lint` o `test` la adopción no sigue — el `verifier` los necesita para abrir el GATE 2 (check 12) |
| Lockfile vs. gestor declarado | Si el stack declara `pnpm` y el repo tiene `package-lock.json` (o al revés), dilo en el checkpoint: `<audit>` va a fallar con `ERR_PNPM_AUDIT_NO_LOCKFILE` y el `verifier` va a correr el audit del otro gestor y reportar GAP en cada PR. Migrar el lockfile es tarea de la adopción, no del primer PR de feature; si no se migra ahora, va al backlog con esa razón escrita |
...
Copia `templates/settings.json` del stack instalado a `AGENTS.md` (reemplaza el actual; conserva en `allow` solo los permisos propios del repo que sigan haciendo falta, por ejemplo servidores MCP o scripts locales; no conserves hooks viejos). Si no hay plugin de stack, escribe un `settings.json` mínimo con `includeCoAuthoredBy: false` y los `deny` de `.env*`. Copia `templates/sdd-hooks.env` de este plugin a `.agentic/sdd-hooks.env` con los valores del checkpoint 0. Crea `<raíz de artefactos>/.gitkeep` solo si el store es `repo`. No crees `specs/` ni `_archive/` a mano: los crea el `archiver` al cerrar el primer change (ver `ORCHESTRATOR.md` §3.1).
...
**Solo con autorización explícita (checkpoint 4)**: elimina orquestadores, comandos, hooks y skills locales que el plugin reemplaza (`.agents/orchestrator/`, `.agents/commands/`, `.agents/hooks/`, `ROUTER.md`, skills `.md` planas y las absorbidas según el informe o tu análisis del paso 4); el AGENTS.md viejo si estaba en `.agents/`; hooks viejos del `settings.json`. Conserva la skill de invariantes y lo que el checkpoint 2 dejó como LOCAL. Corre `<typecheck> && <lint> && <test>` y muestra la salida. Commit `chore(docs): remove superseded orchestrator, commands and skills`.
