...
**Solo con autorización explícita (checkpoint 4)**: elimina orquestadores, comandos, hooks y skills locales que el plugin reemplaza (`.opencode/orchestrator/`, `.opencode/commands/`, `.opencode/hooks/`, `ROUTER.md`, skills `.md` planas y las absorbidas según el informe o tu análisis del paso 4); el AGENTS.md viejo si estaba en `.opencode/`; hooks viejos del `settings.json`. Conserva la skill de invariantes y lo que el checkpoint 2 dejó como LOCAL. Corre `<typecheck> && <lint> && <test>` y muestra la salida. Commit `chore(docs): remove superseded orchestrator, commands and skills`.
