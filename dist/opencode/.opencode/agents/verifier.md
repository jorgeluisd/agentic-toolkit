...
- Comandos del proyecto: sección "Comandos" del `AGENTS.md` (typecheck, lint, test, test:integration, audit, boundaries si está separado).
...
| 3 | Tests unit + handler | `<test>` declarado en `AGENTS.md` §6 | exit 0; **ningún test `skip`/`only`/`todo` nuevo sin justificación en el apply-progress** |
...
| 9 | Seguridad mecánica | `<audit>` del stack. Si falla porque el lockfile no es el del gestor declarado (`ERR_PNPM_AUDIT_NO_LOCKFILE` en un repo con `package-lock.json`), **no se marca FAIL y no se deja sin correr**: se corre el audit del gestor que sí tiene lockfile (`npm audit --audit-level=high`), su resultado vale como el del check, y la migración del lockfile se reporta como GAP con la línea literal del error. Degradar en silencio esconde la deuda; marcar FAIL frena el gate por algo que no introdujo el diff. Además: grep de secretos en el diff (`gitleaks protect --staged` o patrones: `sk-`, `eyJ[A-Za-z0-9_-]{20,}`, `postgres://.*:.*@`); DTOs sin campo de tenant; sin `.env*` en el diff; sin datos personales en `docs/sdd/` ni fixtures | Limpio |
...
| 12 | Configuración faltante | Un comando simbólico que `AGENTS.md` §6 no declara, ni declara como `no aplica` | Regla única, para que dos verifiers no resuelvan lo mismo distinto: **`<typecheck>`, `<lint>` y `<test>` sin declarar son FAIL** — sin ellos los checks 1, 2 y 3 no existen y el gate no tiene sobre qué abrirse. **Los demás (`<test:integration>`, `<build>`, `<audit>`, `boundaries`, `prod_markers`) son FAIL solo si el diff los necesita** (ejemplo: el diff toca repos, esquema o migraciones y no hay `<test:integration>`), y **GAP** en cualquier otro caso. Un `no aplica` explícito en `AGENTS.md` §6 no es GAP: es una decisión tomada. Lo que nunca es válido es dejar el check en N/A sin decir cuál de los tres casos es |
