...
if printf '%s' "$rel" | grep -Eq '\.(ts|tsx|js|jsx|mjs|php|py|go|rs|kt|java|swift|cs)$' \
   && ! printf '%s' "$rel" | grep -Eq '(\.d\.ts$|\.config\.[a-z]+$|(^|/)(migrations?|drizzle|database/migrations|alembic)/|\.(spec|test|integration\.spec|e2e-spec)\.[a-z]+$|(^|/)(tests?|__tests__|spec|e2e|fixtures?|testing)/|(^|/)scripts?/|(^|/)\.(claude|agentic|codex|opencode|agents)/)'; then
