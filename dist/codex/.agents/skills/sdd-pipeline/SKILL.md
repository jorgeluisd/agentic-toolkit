...
Cuando el usuario pide un cambio de código en lenguaje natural ("agrega un endpoint", "arregla el bug de X", "implementa la feature Y", "migra la tabla Z"), **no se implementa directo**. Se hace exactamente lo que haría `/prompts:sdd <objetivo>`: leer `ORCHESTRATOR.md` del plugin, clasificar el nivel y lanzar los agentes en orden con artefactos en `docs/sdd/<NNNN>-<slug>/`.
