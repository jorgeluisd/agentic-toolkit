...
# Configuración: el archivo del proyecto (KEY=VALUE, una por línea) manda. Se busca
# .agentic/sdd-hooks.env y, si no está, .claude/sdd-hooks.env (compatibilidad con
# repos adoptados antes de 2.0.0). El userConfig del plugin
# (CLAUDE_PLUGIN_OPTION_<KEY>, solo en Claude Code) es el fallback, pensado para
# instalaciones a nivel de usuario compartidas por varios repos, donde cada repo
# declara sus propios valores.
#
# Estos hooks corren igual en Claude Code y en Codex CLI: el esquema de payload y
# de decisión es el mismo. En OpenCode los invoca el shim de .opencode/plugins/.
#
# La raíz de la SESIÓN (SDD_PROJECT_DIR, CLAUDE_PROJECT_DIR o cwd) no tiene por
# qué ser un repositorio: abierta en una carpeta que contiene varios repos apunta
# a la contenedora, ahí no está el sdd-hooks.env y la configuración del repo se
...
# _sdd_env_file <raíz>: ruta del archivo de configuración del repositorio, en orden
# de precedencia. .agentic/ es el nombre neutral; .claude/ se sigue aceptando para
# no romper los repos adoptados antes de 2.0.0. Devuelve 1 si no hay ninguno.
_sdd_env_file() {
  local d="$1" f
  for f in .agentic/sdd-hooks.env .claude/sdd-hooks.env; do
    [ -f "$d/$f" ] && { printf "%s" "$d/$f"; return 0; }
  done
  return 1
}

# _sdd_local_store <raíz>: carpeta del store no versionado. .agentic/sdd es el
# nombre neutral; si el repo ya tiene .claude/sdd de una adopción previa, se respeta.
_sdd_local_store() {
  [ -d "$1/.claude/sdd" ] && [ ! -d "$1/.agentic/sdd" ] && { printf "%s" ".claude/sdd"; return 0; }
  printf "%s" ".agentic/sdd"
}
...
_sdd_climb() {
  ...
  [ -d "$d" ] || d="${d%/*}"
  while [ -n "$d" ] && [ "$d" != "/" ]; do
    _sdd_env_file "$d" >/dev/null && { printf "%s" "$d"; return 0; }
...
_sdd_configure() {
  ...
  _end="$(_sdd_env_file "$PROJECT_DIR")"
  if [ -n "$_end" ]; then
  ...
  [ -z "$ARTIFACTS_DIR" ] && { [ "$ARTIFACT_STORE" = repo ] && ARTIFACTS_DIR="docs/sdd" || ARTIFACTS_DIR="$(_sdd_local_store "$PROJECT_DIR")"; }
...
AI_TRAILER_RE='co-authored-by|claude-session|devin-session|generated with|anthropic|noreply@anthropic|openai\.com|devin\.ai'
