...
El pipeline se dispara igual si el pedido llega por `/prompts:sdd` o en lenguaje natural ("agrega un endpoint…"): la skill `sdd-pipeline` y el hook `UserPromptSubmit` del plugin recuerdan clasificar el nivel antes de tocar archivos, y el `AGENTS.md` de cada proyecto lo fija como regla.
