...
El hook `post-bash` del plugin registra cada corrida de test en `docs/sdd/<feature>/tdd-evidence.log`, una línea por corrida: `<ISO-8601> | exit=<n> | <comando> | <resumen del runner>[ | WARN=<marca>]`. Está registrado en `PostToolUse` (la llamada Bash terminó en 0) y en `PostToolUseFailure` (falló), así que un RED queda con su exit code real y el resumen del runner. Ese exit code es el de **la llamada entera**, no el de los tests: si el runner reporta tests ejecutados y ninguno rojo pero otra parte del comando falló, la línea queda `exit=0` con `WARN=call-failed-outside-tests`; si los tests ni llegaron a correr, `WARN=no-tests-ran`. Solo cuando no llega ningún evento del harness la corrida se concilia como `| exit=!0 | <comando> | sin salida capturada | WARN=resultado-inferido-por-ausencia-de-PostToolUse`, y la línea de salida del RED se toma de lo que devolvió el comando en la conversación.
...
La tabla del apply-progress **referencia esos timestamps**, no hashes de commit: la evidencia sobrevive a squash y rebase. En la columna Comando va el comando real ya resuelto (el que el hook registró), no el símbolo.
