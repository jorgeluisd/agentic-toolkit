...
| 12 | Configuración faltante | Un comando simbólico que `CLAUDE.md` §6 no declara, ni declara como `no aplica` | Regla única, para que dos verifiers no resuelvan lo mismo distinto: **`<typecheck>`, `<lint>` y `<test>` sin declarar son FAIL** — sin ellos los checks 1, 2 y 3 no existen y el gate no tiene sobre qué abrirse. **Los demás (`<test:integration>`, `<build>`, `<audit>`, `boundaries`, `prod_markers`) son FAIL solo si el diff los necesita** (ejemplo: el diff toca repos, esquema o migraciones y no hay `<test:integration>`), y **GAP** en cualquier otro caso. Un `no aplica` explícito en `CLAUDE.md` §6 no es GAP: es una decisión tomada. Lo que nunca es válido es dejar el check en N/A sin decir cuál de los tres casos es |
