...
Cada proyecto los copia en su `CLAUDE.md` §6; si difieren (turbo, filtros), gana el `CLAUDE.md`. Un símbolo que el proyecto no tiene se declara `no aplica` con el porqué: sin declarar, el `verifier` no puede distinguir una decisión de un olvido (check 12).
