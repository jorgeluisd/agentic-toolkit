...
- Skills según la tarea: siempre `onion-screaming-architecture` y `errors-and-result`; `strict-tdd` si `TDD: ON`; las demás según el `tipo` (`domain-modeling`, `application-cqrs-jobs`, `persistence-drizzle`, `multi-tenancy-rls`, `frontend-next-react`, `logging-pino`, `security-baseline`). Skill local de invariantes del proyecto.
