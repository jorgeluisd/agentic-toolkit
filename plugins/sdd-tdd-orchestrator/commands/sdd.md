...
4. Nivel `full` (default): explorer → proposer → spec-writer → designer → task-planner → **GATE 1** → implementer por tarea (strict-tdd si `TDD: ON`) → verifier ∥ security-reviewer (si hubo riesgos) → `/pr-draft` → **GATE 2** → (merge humano) → archiver.
   Nivel `bugfix`: explorer → implementer (TDD ON, el test que reproduce el bug es el RED) → verifier → **GATE 2**. Si aparece cambio de contrato o esquema, sube a `full`.
...
6. Un FAIL del verifier vuelve al implementer con la tabla; nunca llega al GATE 2.
