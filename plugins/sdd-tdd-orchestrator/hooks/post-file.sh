...
if printf '%s' "$rel" | grep -Eq '(migrations?|drizzle)/.*\.sql$'; then
  up="$(tr '[:lower:]' '[:upper:]' < "$path")"
  printf '%s' "$up" | grep -Eq 'DROP[[:space:]]+(TABLE|COLUMN)|ALTER[[:space:]]+COLUMN[[:space:]]+[^;]*[[:space:]]TYPE[[:space:]]|TRUNCATE[[:space:]]' && problems="$problems
