...
if printf '%s' "$lc" | grep -Eq '(^|[[:space:];&|])(fly(ctl)?|vercel|railway|gcloud|eb|heroku)[[:space:]]+deploy|vercel[[:space:]].*--prod|supabase[[:space:]]+(db[[:space:]]+(push|reset)|link)|drizzle-kit[[:space:]]+(push|migrate)|db:(push|migrate|reset)|prisma[[:space:]]+(db[[:space:]]+push|migrate[[:space:]]+deploy)|flyctl?[[:space:]]+secrets|fly[[:space:]]+ssh'; then
...
if printf '%s' "$lc" | grep -Eq '(^|[[:space:];&|])(npm|yarn)[[:space:]]+(install|add|i|ci|run|exec)'; then
  deny "El gestor de paquetes del estándar es pnpm. Usa pnpm."; exit 0
fi
if printf '%s' "$lc" | grep -Eq 'pnpm[[:space:]]+(add|install)[[:space:]]' && printf '%s' "$lc" | grep -Eq '(--unsafe-perm|dangerouslyallowallbuilds|strict-dep-builds=false)'; then
  deny "Aprobación masiva de scripts de instalación prohibida. Usa pnpm approve-builds con OK humano por paquete."; exit 0
fi
